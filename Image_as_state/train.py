...
# used to ensure that you can restart at any epoch. If required
...
for epoch_num in range(current_epoch, total_epochs):
    ...
    for idx, (image, target_capt, seqlen, input_capt) in enumerate(train_loader):
        target_capt = \
        torch.nn.utils.rnn.pack_padded_sequence(target_capt, seqlen, batch_first=True)[0]
        ...
        if image.shape[0] == 1:
            continue
        optimizer.zero_grad()
        output = model(image.float().to(device), input_capt.long().to(device),
